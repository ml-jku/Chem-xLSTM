"""
adapted from https://github.com/molML/s4-for-de-novo-drug-design/blob/main/s4dd/smiles_utils.py
"""
...
_ELEMENTS_STR = r"(?<=\[)Cs(?=\])|Si|Xe|Ba|Rb|Ra|Sr|Dy|Li|Kr|Bi|Mn|He|Am|Pu|Cm|Pm|Ne|Th|Ni|Pr|Fe|Lu|Pa|Fm|Tm|Tb|Er|Be|Al|Gd|Eu|te|As|Pt|Lr|Sm|Ca|La|Ti|Te|Ac|Cf|Rf|Na|Cu|Au|Nd|Ag|Se|se|Zn|Mg|Br|Cl|Pb|Sb|U|V|K|C|B|H|N|O|S|P|F|I|b|c|n|o|s|p"
__REGEXES = {
...
def pad_sequences(
    ...
    truncated means that the first tokens are removed.
    ...
    warn: bool
        If True, a warning is printed if a sequence is longer than the padding_length.
    Returns
    -------
    List[List[Union[str, int]]
    ...
    if warn:
        if any([len(seq) > padding_length for seq in sequences]):
            print(
                "Warning: Some sequences are longer than the padding length. They will be truncated at the front."
            )
...
def augment_smiles(smiles: str, isomericSmiles=True, kekuleSmiles=False) -> str:
    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        return smiles
    new_smiles = Chem.MolToSmiles(mol, canonical=False, isomericSmiles=isomericSmiles, kekuleSmiles=kekuleSmiles)
    return new_smiles

def augment_smiles_batch(smiles_batch: List[str], isomericSmiles=True, kekuleSmiles=False) -> List[str]:
    return [augment_smiles(smiles, isomericSmiles, kekuleSmiles) for smiles in smiles_batch]